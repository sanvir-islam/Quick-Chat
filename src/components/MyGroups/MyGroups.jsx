...
function MyGroups() {
  ...
  async function handleGroupJoinRequestAccept(reqId, senderId) {
    try {
      const group = myGroupList.find((group) => group.id === reqId);
      await handleGroupJoinRequestRemove(reqId);
      //update the members list
      await writeDataInDb(`groups/${reqId}`, { ...group, members: [...group.members, senderId] });
    } catch (err) {
      console.log(err.message);
    }
  }
    ...
    <div className="pl-[20px] pr-[15px] pb-[15px] pt-[19px] h-[463px] rounded-[20px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] w-full mt-[43px]">
      ...
      <div className="flex justify-between align-middle mb-[35px] ">
        ...
        <div className="flex justify-between ">
          ...
          <button className="ml-3">{!showGroupJoinReq && <BsThreeDotsVertical size={20} />}</button>
        </div>
      </div>
      ...
      {showGroupJoinReq ? (
        <div className=" h-[320px] w-[344px] pr-[15px] scrollbar-custom ">
          {myGroupJoinReqList.length === 0 ? (
            <div className="w-[260px] h-[340px] p-4 mx-auto opacity-50">
              ...
              return (
                <div className="relative flex justify-between align-middle mb-[32px] " key={groupJoinReq.id}>
                  ...
                  <div className="flex justify-between gap-4 align-middle">
                    ...
                    <button onClick={() => handleGroupJoinRequestAccept(groupJoinReq.id, groupJoinReq.senderid)}>
